...
WHEEL_RADIUS = 0.0252#轮子半径
DISTANCE_BETWEEN_WHEELS = 0.32#两轮机器人两个轮子之间的距离
ROBOT_RADIUS = 0.3#机器人半径
POS_TOLERANCE = 0.03#允许的位置误差范围
ANGLE_TOLERANCE = 0.06#允许的航向角误差范围
POS_REQUEST_RATE = 30.0#向slam节点请求位姿的频率
PROCESS_COSTMAP = False #源代码不完善没有代码用到这个参数,设置为True会报错
ROTATE_AROUND_GRANULARITY = 9#原地旋转360度分成9份,每份旋转40度,没有什么意思
LINEAR_VELOCITY = 0.15 * 3#线速度
OBSTACLE_DETECTION_THRESHOLD = 0.50 #检测是否遇到障碍的阈值
# 0.75
        ...
        while not obstacleEncountered and wasLocalGoalDefined:
            ...
            if minRange < OBSTACLE_DETECTION_THRESHOLD:
                ...
                if diff1 >= 0 and diff2 >= 0:
                    ...
                    #判断是否遇到障碍了,如果激光点距离在目标点距离和机器人半径之间而且距离机器人的横向距离小于机器人半径就计为遇到了障碍
...
def exploreEnvironment():
    ...
    teleop_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=5)
    ...
    while not rospy.is_shutdown() and not exit:
        ...
        print "current position X: %f Y: %f."%(current_x,current_y)
...
if __name__ == "__main__":
    ...
    direction = raw_input("Please input the map file save direction (x for abort): ")
    
    if  direction != 'x' and direction != 'X':
        file_name = raw_input("Please input the map file name : ")
        print "Saving file: %s (.png & .yaml) to direction: %s" %(file_name, direction)
        map_save_cmd = "rosrun map_server map_saver -f %s/%s" %(direction, file_name)
        os.system(map_save_cmd)
    else :    
        pass
    
    exit = True
    print "AUTO EXPLORARION TASK DONE !!!"
